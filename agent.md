...
- **`worker/`**: Cloudflare Worker (Wasm). Exposes `/status` API endpoints.
- **`importer/`**: GitHub Actions standalone CLI. Connects directly to Cloudflare D1 via HTTP APIs to ingest ZIP/CSV GTFS files.
...
- Agents deploying or configuring databases must utilize the unified `./deploy.sh` orchestrator which provisions D1, runs migrations, and triggers `generate-wrangler.sh`. 
...
- **Global Concurrency Limits**: Rely on `Arc<Semaphore>` across providers and tasks to bound memory (e.g., parsing large ZIP files) and network connections. Prevents OOM crashes from unbound file task processing.
